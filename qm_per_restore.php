...
if ( !empty($_POST)) { // if user clicks "yes" (sure to delete), delete record
	...
	$sql = "UPDATE qm_persons  set archive_flag = false WHERE id = ?";
	$q = $pdo->prepare($sql);
	$q->execute(array($id));
	Database::disconnect();
	header("Location: qm_per_list_archive.php");
...
<body style ="background-color: lightblue !important";>
    <!-- <div class="container">
		...
		<table class="table table-striped table-bordered" style="background-color: lightgrey !important">
		 ...
		 <div class="container">
		<div class="row">
			<h3>Restore Person</h3>
		</div>
		
		<form class="form-horizontal" action="qm_per_restore.php" method="post">
			<input type="hidden" name="id" value="<?php echo $id;?>"/>
			<p class="alert alert-error">Are you sure you want to Restore ?</p>
			<div class="form-actions">
				<button type="submit" class="btn btn-success">Yes</button>
				<a class="btn btn-danger" href="qm_per_list_archive.php">No</a>
			</div>
		</form>
		<br>
		...
		<div class="form-horizontal" >
			...
			<p>Page created by: Aashish Shrestha, ashrest8@svsu.edu</p>
