<?php  
/* ---------------------------------------------------------------------------
 * filename    : qm_quix_archive.php
 * author      :
 * description : This program archives one quiz's details (table: qm_quizzes)
 * ---------------------------------------------------------------------------
 *
session_start();
...
if ( !empty($_POST)) { // if user clicks "yes" (sure to delete), delete record
	...
	$sql = "SELECT archive_flag FROM qm_quizzes where id = ?";
	$q = $pdo->prepare($sql);
	$q->execute(array($id));
	$data = $q->fetch(PDO::FETCH_ASSOC);
	Database::disconnect();
	if ($data['archive_flag'] == 0){ 
		$archive = 1;
		$pdo = Database::connect();
		$pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
		$sql = "UPDATE qm_quizzes SET archive_flag =? WHERE id = ?";
		$q = $pdo->prepare($sql);
...
<body>
    <div class="container">
		...
		<form class="form-horizontal" action="qm_quiz_archive.php" method="post">
			<input type="hidden" name="id" value="<?php echo $id;?>"/>
			<p class="alert alert-error">Are you sure you want to archive/unarchive?</p>
